...
//            errorDialog.show();
            ...
            noDataLayout.setActionButton("HOME", new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    onBackPressed();
                }
            });
            noDataLayout.setMessage(getString(R.string.no_fees));
            noDataLayout.show();
        }
    ...
    FeesDewListAdapter.TouchEvents touchEvents = new FeesDewListAdapter.TouchEvents() {
        ...
        public void onChecked(int position) {
            for (int i = 0; i <= position; i++) {
                feeStructureArray.get(i).setSelected(true);
            }
            feesDewListAdapter.notifyDataSetChanged();
        }
        ...
        public void onUnchecked(int position) {
            for (int i = position; i < feeStructureArray.size(); i++) {
                feeStructureArray.get(i).setSelected(false);
            }
            feesDewListAdapter.notifyDataSetChanged();
        }
    };
...
//            Utils.showShortToast(context, amount+"");
            Intent gatewayIntent = new Intent(context, PaymentGatewayActivity.class);
